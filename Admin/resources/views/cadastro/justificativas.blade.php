                        ...
                        <table id="table_justificativa" class="table " style="white-space: nowrap;">
                            ...
                            @foreach($justificativas as $justificativa)
                                <tr id="{{ $justificativa->CODIGO }}">
                                    <td id="{{ "datacad_".$justificativa->CODIGO}}"> {{date("d/m/Y", strtotime($justificativa->DATA_CADASTRO))}} </td>
...
<script>
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $.ajax({
                    ...
                    header:{'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function(response){
                        let node = document.getElementById("conteudo_tabe");
                        $("#conteudo_tabe").empty();

                        let html = "";

                        for(let i=0; i < response.length; i++){
                            let data = new Date(response[i].DATA_CADASTRO);

                            html = "<tr id='" + response[i].CODIGO + "'>";
                                html += "<td id='datacad_" + response[i].CODIGO + "'>" + data.toLocaleDateString() + "</td>";
    ...
    function salvarEdicaoJustificativa(){
        ...
        let url = "{{ url('edit-justificativa') }}" + "/" + justificativa;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                linhas[0].cells[1].textContent = justificativa.justificativa;
